...
class KFoldStrategyWithCondFile(SetupStrategy):
    def __init__(self, fold, folds=5, shuffle=False) -> None:
        assert 0 <= fold < folds
        self.fold = fold
        self.folds = folds
    ...
    def setup(self, train_ds, val_ds, test_ds) -> None:
        ...
        if self.shuffle:
            log.info(
                f"Dataset has been shuffled before splitting into train and validation sets."
            )
        else:
            log.info(
                f"Dataset has NOT been shuffled before splitting into train and validation sets."
            )
