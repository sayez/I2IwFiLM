...
# %%
def dump_onesample_pair(args):
    # zip(range(len(selected_times)), time_data_mapping.items(), 
    #                                     repeat(data_aia), repeat(data_hmi), 
    #                                     repeat(selected_aia_indices), repeat(selected_hmi_indices),
    #                                     repeat(output_aia_folder), repeat(output_hmi_folder), 
    #                                     repeat(redo_all), repeat(save), repeat(show))
    i, (d, cur), data_aia, data_hmi, selected_aia_indices, selected_hmi_indices, output_aia_folder, output_hmi_folder, redo_all, save, show = args
...
def main(year, output_folder, redo_all=False):
    ...
    with Pool(num_cores) as p:
        list(tqdm(p.imap(dump_onesample_pair, zip(range(len(selected_times)), time_data_mapping.items(), 
                                        repeat(data_aia), repeat(data_hmi), 
                                        repeat(selected_aia_indices), repeat(selected_hmi_indices),
                                        repeat(output_aia_folder), repeat(output_hmi_folder), 
                                        repeat(redo_all), repeat(save), repeat(show))
        ), total=len(selected_times)))
